...
| Convolution 2:            | 64 Filters, kernal 5x5, activation: elu, strides 2x2  |
...
Additionally, the time for each epoch had to be serverely reduced in order to complete the project. This was done by dividing the number of samples_per_epoch by dividing the training samples by a factor of 2. The number of epochs had to also be cut down in order to achieve a training set to use on the autonomous vehicle in a reasonable time period.
